...
def aggregate(reports, stores_cfg, prev_close=None, prev_reports=None):
    ...
    for org, rpt in list(reports.items()) + \
                    [(o, r) for o, r in prev_reports.items() if o not in reports]:
        carried = org not in reports
        info = store_lookup.get(org)
        if not info:
            continue
        for p in rpt.get("개인별", []):
            rec = dict(p)
            rec["조직"], rec["상권"], rec["이월"] = org, info["상권"], carried
            persons.append(rec)
    persons.sort(key=lambda p: (p["실적"] / p["목표"]) if p["목표"] else 0, reverse=True)
    ...
    result["개인"] = persons
    result["개인지사계"] = ptotal
    result["개인미제출매장"] = [s["조직"] for s in result["매장_정렬"]
                                if s.get("데이터있음")
...
def build_workbook(agg, date_str, out_path, title="■ 강동소매 폴더블8 매장별 예약현황"):
    ...
    if agg.get("개인"):
        ...
        for p in agg["개인"]:
            write_person(r, p, "person"); r += 1
        miss = agg.get("개인미제출매장")
        if miss:
            _style(ws3, f"B{r + 1}",
                   f"＊ 이월(매장 미제출) / 개인별 미제출 매장: {', '.join(miss)}",
