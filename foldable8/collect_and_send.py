...
def build_error_check(prev_reports, now):
    """전일 보고의 검증오류를 3분류해 점검 안내 메시지 생성.
    오류가 없으면 '오류 없음' 메시지 반환."""
    cat_a, cat_b, cat_c = [], [], []
    for name, v in prev_reports.items():
        errs = v.get("검증오류", [])
        if not errs:
            continue
        a = any(("모델별 합" in e) or ("실적 합" in e) for e in errs)
        b = any(("숫자" in e) and ("개" in e) for e in errs)
        c = any(not (("모델별 합" in e) or ("실적 합" in e)
                     or (("숫자" in e) and ("개" in e))) for e in errs)
        if a:
            cat_a.append(name)
        if b:
            cat_b.append(name)
    ...
    total = sum(1 for v in prev_reports.values() if v.get("검증오류"))
    if total == 0:
        return ("✅ 전일 입력 오류 없음\n"
                "어제 보고는 전 매장 정상 입력되었습니다. 오늘도 정확한 입력 부탁드립니다!")
 
    fmt = lambda lst: "·".join(lst)
    msg = ("⚠️ 전일 입력 점검 안내\n"
           "어제 보고에서 아래 매장에 수치 불일치가 있었습니다. "
           "오늘 보고 시 한 번 더 확인 후 공유 부탁드립니다.\n")
    if cat_a:
        msg += ("\n① 예약 계 불일치 (모델별 합·개인 실적 합 ≠ 예약)\n"
                f" → {fmt(cat_a)}\n")
    if cat_b:
        msg += ("\n② 개인별 보고항목 입력개수 확인 (한 명당 12개)\n"
                f" → {fmt(cat_b)}\n")
    if cat_c:
        msg += ("\n③ 연계실적 개인 배분 불일치 (120K·2nd·카드 등)\n"
                f" → {fmt(cat_c)}\n")
    return msg
...
def main(mode="report"):
    ...
    # 리마인드 모드 (19시): 취합 안내 공지 + 전일 입력 점검 안내
    if mode == "remind":
